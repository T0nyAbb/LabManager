...
public class TecnicoDao implements Dao<Tecnico> {
    ...
    public List<Tecnico> getAll() {
        ...
        try{
            ...
            while(rs.next()) {
                for(Sede sede:sedi) {
                    if(rs.getString("TECNICO.ID_SEDE").equals(rs.getString("S.ID_SEDE")) && sede.getIndirizzo().equals(rs.getString("S.INDIRIZZO"))) {
                        Tecnico t = new Tecnico(sede, rs.getString("MATRICOLA"), rs.getString("NOME"), rs.getString("COGNOME"), rs.getDate("DATANASCITA"), rs.getString("CODICEFISCALE"), rs.getString("TECNICO.INDIRIZZO"), rs.getString("TELEFONO1"), rs.getString("TELEFONO2"), rs.getString("EMAIL"));
                        tecnico.add(t);
                    }
                }
