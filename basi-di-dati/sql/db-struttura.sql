...
--VISTE PER IL RIEPILOGO MENSILE DI PRENOTAZIONI STRUMENTO
...
--VIEW: MOSTRA L'UTENTE CHE HA EFFETTUATO PIU' PRENOTAZIONI IN BASE ALLO STRUMENTO PRENOTATO ED AL MESE IN CUI E' STATO USATO
CREATE OR REPLACE VIEW TOP_PRENOTATORE_PER_STRUMENTO_MESE(ID_STRUMENTO, MESE, N_PRENOTAZIONI, USERNAME) AS
SELECT ID_STRUMENTO, MESE, N_PRENOTAZIONI, P2.USERNAME
FROM(
...
CREATE OR REPLACE VIEW CONT_PRENOTAZIONI_PER_STRUMENTO_MESE(ID_STRUMENTO, MESE, N_PRENOTAZIONI_TOTALI) AS
...
SELECT DISTINCT TP.ID_STRUMENTO, TP.MESE, TP.USERNAME, TP.N_PRENOTAZIONI
...
SELECT DISTINCT TP.ID_STRUMENTO, TP.MESE, TP.USERNAME, TP.ORE_UTILIZZO
FROM TOP_UTILIZZATORE_PER_STRUMENTO_MESE TP
JOIN CONT_UTILIZZO_STRUMENTO_MESE_UTENTE CU on TP.ID_STRUMENTO = CU.ID_STRUMENTO
ORDER BY MESE, ID_STRUMENTO;




--VISTE PER IL RIEPILOGO ANNUALE DI PRENOTAZIONI STRUMENTO
...
CREATE OR REPLACE VIEW TOP_PRENOTATORE_PER_STRUMENTO_ANNO(ID_STRUMENTO, ANNO, USERNAME) AS
SELECT ID_STRUMENTO, ANNO, P2.USERNAME
FROM(
...
CREATE OR REPLACE VIEW RIEPILOGO_STRUMENTO_ANNO(ID_STRUMENTO, ANNO, N_PRENOTAZIONI, TOP_UTENTE) AS
SELECT DISTINCT ID_STRUMENTO, ANNO, CP.N_PRENOTAZIONI, TP.USERNAME
FROM TOP_PRENOTATORE_PER_STRUMENTO_ANNO TP
NATURAL JOIN CONT_PRENOTAZIONI_PER_STRUMENTO_ANNO CP
ORDER BY ID_STRUMENTO, ANNO;
...
CREATE OR REPLACE VIEW CONT_UTILIZZO_STRUMENTO_MESE_UTENTE(ID_STRUMENTO, MESE, ORE_UTILIZZO, USERNAME) AS
SELECT ID_STRUMENTO, MESE, SUM(DURATA), USERNAME FROM
                    (
...
--VIEW: MOSTRA L'UTENTE CHE HA UTILIZZATO DI PIU' UN DETERMINATO STRUMENTO IN UN DETERMINATO MESE
CREATE OR REPLACE VIEW TOP_UTILIZZATORE_PER_STRUMENTO_MESE(ID_STRUMENTO, MESE, USERNAME, ORE_UTILIZZO) AS
SELECT ID_STRUMENTO, MESE, P2.USERNAME, ORE_UTILIZZO
FROM(
        SELECT ID_STRUMENTO, MESE, MAX(ORE_UTILIZZO) AS MAX_ORE_UTILIZZO
        FROM CONT_UTILIZZO_STRUMENTO_MESE_UTENTE
        GROUP BY ID_STRUMENTO, MESE
    )P1
        NATURAL JOIN(
