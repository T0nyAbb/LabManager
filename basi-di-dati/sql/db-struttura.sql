...
CREATE TABLE PRENOTAZIONE(
    ...
    --FOREIGN KEY
    CONSTRAINT prenotazione_fk_strumento FOREIGN KEY (ID_STRUMENTO) REFERENCES STRUMENTO(ID_STRUMENTO) ON DELETE CASCADE,
    CONSTRAINT prenotazione_fk_utente FOREIGN KEY (USERNAME) REFERENCES UTENTE(USERNAME) ON DELETE CASCADE,
...
CREATE OR REPLACE FUNCTION autenticazione(usrname VARCHAR2, pwd VARCHAR2)
RETURN VARCHAR2 AS
tmp UTENTE.USERNAME%TYPE;
BEGIN
SELECT USERNAME INTO tmp
FROM UTENTE
WHERE UPPER(usrname) = UPPER(UTENTE.USERNAME) AND decrypt_pwd(UTENTE.USR_PASSWORD) = LTRIM(RTRIM(pwd));
RETURN 0;
EXCEPTION
...
CREATE OR REPLACE VIEW TOP_PRENOTATORE_PER_STRUMENTO_MESE(ID_STRUMENTO, MESE, USERNAME) AS
SELECT ID_STRUMENTO, MESE, P2.USERNAME
FROM(
...
SELECT ID_STRUMENTO, MESE, SUM(N_PRENOTAZIONI)
FROM CONT_PRENOTAZIONI_PER_STRUMENTO_MESE_UTENTE
GROUP BY ID_STRUMENTO, MESE;

--VIEW: MOSTRA IL RIEPILOGO MENSILE DI UNO STRUMENTO (IL NUMERO TOTALE DI PRENOTAZIONI E L'UTENTE CON PIU' PRENOTAZIONI PER UNO STRUMENTO IN UN DETERMINATO MESE)
CREATE OR REPLACE VIEW RIEPILOGO_STRUMENTO_MESE(ID_STRUMENTO, MESE, N_PRENOTAZIONI, TOP_UTENTE) AS
SELECT DISTINCT ID_STRUMENTO, MESE, CP.N_PRENOTAZIONI, TP.USERNAME
FROM TOP_PRENOTATORE_PER_STRUMENTO_MESE TP
NATURAL JOIN CONT_PRENOTAZIONI_PER_STRUMENTO_MESE CP
ORDER BY ID_STRUMENTO, MESE;
...
--VIEW: MOSTRA IL RIEPILOGO ANNUALE DI UNO STRUMENTO (IL NUMERO TOTALE DI PRENOTAZIONI E L'UTENTE CON PIU' PRENOTAZIONI PER UNO STRUMENTO IN UN DETERMINATO ANNO)
